...
export default {
  ...
  watch: {
    message(value) {
      if (value) {
        Echo.private(`Chats.${this.friend.session.id}`).whisper("typing", {
          name: auth.name
        });
      }
    }
  ...
  created() {
    ...
    Echo.private(`Chats.${this.friend.session.id}`).listen(
      "PrivateChatEvent",
      e => {
    ...
    Echo.private(`Chats.${this.friend.session.id}`).listen("MsgReadEvent", e =>
    ...
    Echo.private(`Chats.${this.friend.session.id}`).listenForWhisper(
      "typing",
      e => {
        this.isTyping = true;
        setTimeout(() => {
